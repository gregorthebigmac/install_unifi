...
# UniFi Controller 5.6.42 auto installation script.
# OS       | Bionic
# Version  | 3.9.4
...
if [ $(cat /root/.bashrc | grep "^export PATH=" | grep -c "/sbin") -eq 0 ]; then
  if [[ $(cat /root/.bashrc | grep "^export PATH=") ]]; then
...
if [ $(dpkg-query -W -f='${Status}' sudo 2>/dev/null | grep -c "ok installed") -eq 0 ]; then
  ...
  if [[ $? > 0 ]]; then
    if [[ $(find /etc/apt/* -name *.list | xargs cat | grep -P -c "^deb http://[A-Za-z0-9]*.archive.ubuntu.com/ubuntu bionic main") -eq 0 ]]; then
...
if [[ $OS_RELEASE != "18.04" && $OS_DESC != "Linux Mint 19"* ]]; then
  echo -e "${RED}################################################################################################################${RESET}"
  echo ""
...
# Check for 64/32 bit
if [[ $ARCHITECTURE != 'x86_64' && $ARCHITECTURE != 'armv8'* && $ARCHITECTURE != 'aarch64' && $ARCHITECTURE != 'arm64' ]]; then
  clear
  echo -e "${RED}########################################################${RESET}"
  echo -e "${RED}#                                                      #${RESET}"
...
if [ $MONGODB_SERVER_INSTALLED -eq 1 ]; then
  ...
  if [ ! -z "$http_proxy" ]; then
    ...
    echo ""
    echo ""
    apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --keyserver-options http-proxy=${http_proxy} --recv-keys 0C49F3730359A14518585931BC711F9BA15703C6
    ...
    if [[ apt_http_proxy ]]; then
      ...
      echo -e "${GREEN}#########################################################################${RESET}"
      echo ""
      echo -e "${GREEN}#${RESET} HTTP Proxy found. | ${WHITE}${apt_http_proxy}${RESET}"
      echo ""
      echo ""
      apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --keyserver-options http-proxy=${apt_http_proxy} --recv-keys 0C49F3730359A14518585931BC711F9BA15703C6
    fi
  else
...
if [[ $JAVA9 -eq 1 || $JAVA10 -eq 1 || $JAVA11 -eq 1 || $JAVA12 -eq 1 ]]; then
...
echo -e "${GREEN}#         ${RESET}Installing UniFi Controller 5.6.42!          ${GREEN}#${RESET}"
echo -e "${GREEN}#                                                      #${RESET}"
echo -e "${GREEN}########################################################${RESET}"
echo ""
sleep 2
...
wget https://dl.ui.com/unifi/5.6.42/unifi_sysvinit_all.deb || abort
...
then
  ...
  if [ ! -z "$http_proxy" ]; then
    ...
    echo ""
    echo ""
    apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --keyserver-options http-proxy=${http_proxy} --recv-keys 06E85760C0A52C50
  elif [ -f /etc/apt/apt.conf ]; then
	apt_http_proxy=$(grep http.*Proxy /etc/apt/apt.conf | awk '{print $2}' | sed 's/[";]//g')
    if [[ apt_http_proxy ]]; then
      clear
      ...
      echo ""
      echo ""
      apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --keyserver-options http-proxy=${apt_http_proxy} --recv-keys 06E85760C0A52C50
    fi
  else
...
if [[ $(dpkg -l | grep "unifi " | grep -c "ii") -eq 1 ]]; then
  ...
  echo ""
  echo ""
  echo -e "${GREEN}#${RESET} UniFi Network Controller 5.6.42 has been installed successfully"
  ...
  echo ""
  echo ""
  echo " Failed to successfully install UniFi Network Controller 5.6.42"
  echo ""
